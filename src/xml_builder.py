...
class XMLBuilder:
    ...
    # ─────────────────────────────────────────────
    # Schema-correct insertion helpers
    # ─────────────────────────────────────────────
    #
    # MusicXML 3.0 requires this order under <score-partwise>:
    #   work → movement-number → movement-title → identification →
    #   defaults → credit* → part-list → part+
    #
    # We use these tag-order indices to insert new top-level children at
    # the right position so the document remains schema-valid even when
    # the caller adds elements in a different order from the spec.
    ...
    def _insert_top_level(self, tag: str) -> etree._Element:
        """
        Create a top-level <tag> child of <score-partwise>, inserting it at
        the position required by the MusicXML schema.  Returns the new
        element.
        """
        order = {t: i for i, t in enumerate(self._TOP_ORDER)}
        target_idx = order[tag]
        # Find the index where the new element should go: just before the
        # first existing child whose order index is greater than target.
        insert_at = len(self._root)
        for i, existing in enumerate(self._root):
            ex_tag = existing.tag
            if ex_tag in order and order[ex_tag] > target_idx:
                insert_at = i
                break
        el = etree.Element(tag)
        self._root.insert(insert_at, el)
        return el
    ...
    def set_defaults(self,
        ...
        The element is inserted at the schema-required position
        (between <identification> and <part-list>) — see
        ``_insert_top_level``.
        """
        defaults = self._insert_top_level('defaults')
    ...
    def _get_or_create_identification(self):
        """Return the <identification> element, creating it if absent.

        <identification> belongs in the schema-required slot between
        <movement-title> (if any) and <defaults>; ``_insert_top_level``
        handles the positioning.
        """
        ident = self._root.find('identification')
        if ident is None:
            ident = self._insert_top_level('identification')
        return ident

    def _get_or_create_miscellaneous(self):
        """Return the <miscellaneous> element inside identification."""
        ident = self._get_or_create_identification()
        misc = ident.find('miscellaneous')
        if misc is None:
            misc = etree.SubElement(ident, 'miscellaneous')
        return misc

    def add_miscellaneous_field(self, name: str, value: str):
        """
        Add  <miscellaneous-field name="NAME">VALUE</miscellaneous-field>
        inside <identification><miscellaneous>.

        Used for free-form metadata: source image filename, OMR coordinate
        dumps, version tags, etc.  Values can be any string (commonly a
        stringified JSON document).

        Multiple fields with the same name are allowed.
        """
        misc = self._get_or_create_miscellaneous()
        f = etree.SubElement(misc, 'miscellaneous-field', name=name)
        f.text = str(value)
        return f

    def set_creator(self, name: str, creator_type: str = 'composer'):
        """
        Add a <creator type="composer">NAME</creator> entry. Convenience.
        creator_type is usually 'composer', 'lyricist', or 'arranger'.
        """
        ident = self._get_or_create_identification()
        c = etree.SubElement(ident, 'creator', type=creator_type)
        c.text = name
        return c
    ...
    def add_note(self,
                 ...
                 # Element id (for cross-referencing from miscellaneous, etc.)
        ...
        slur_start  — True if a slur starts on this note
        slur_stop   — True if a slur ends on this note
        slur_number — slur ID (1-6); use different numbers for nested slurs
        ...
        if tie_start or tie_stop or slur_start or slur_stop:
            ...
            # Slur stop comes before start when both are on the same note
            # (this is the MusicXML convention for chained slurs)
            if slur_stop:
                etree.SubElement(notations, 'slur',
                                 number=str(slur_number), type='stop')
            if slur_start:
                etree.SubElement(notations, 'slur',
                                 number=str(slur_number), type='start')
    ...
    def add_rest(self,
        ...
        measure_rest=True  — adds measure="yes" attribute to <rest> (whole-bar rest).
                             Used for the "skip this measure" fallback case.
        duration_type      — When given (e.g. 'quarter', 'eighth') a proper
                             <type> child is emitted so renderers display the
                             correct rest glyph. Omit for a bare measure-rest.
        element_id         — id attribute on the note element (for cross-ref).
    ...
    def add_grace_note(self,
                       ...
                       element_id:   Optional[str]    = None):
        """
        Add a <note> with a <grace/> element — used for acciaccatura
        (slash=True) and long appoggiatura (slash=False).
        ...
        n = etree.SubElement(measure, 'note', **note_attrs)
        grace_attrs = {'slash': 'yes'} if slash else {}
        etree.SubElement(n, 'grace', **grace_attrs)

        pitch = etree.SubElement(n, 'pitch')
        _sub(pitch, 'step', step.upper())
        if alter is not None and alter != 0:
            _sub(pitch, 'alter', str(int(alter)
                                     if alter == int(alter) else alter))
        _sub(pitch, 'octave', str(octave))
        ...
        if duration_type is not None:
            _sub(n, 'type', duration_type)
        if stem is not None:
            s = etree.SubElement(n, 'stem')
            s.text = stem

        return n

    # ─────────────────────────────────────────────
    # NOTATIONS  (fermata, ornaments, glissando)
    # ─────────────────────────────────────────────

    @staticmethod
    def _get_or_create_notations(note):
        """Return the <notations> child of a note, creating it if absent.

        <notations> must appear AFTER children like <duration>, <voice>,
        <type>, <stem>, <beam>, so we always append at the end of the note.
        """
    ...
    @staticmethod
    def _get_or_create_ornaments(note):
        """Return the <notations><ornaments> child, creating it if absent."""
        notations = XMLBuilder._get_or_create_notations(note)
    ...
    def add_fermata(self,
        ...
        """
        Attach a <fermata> to an existing note element.

        type   — 'upright' (default, hat-down arc above the note) or
                 'inverted' (below the note).
        shape  — Optional shape text: 'normal', 'square', 'angled'.
        """
        notations = self._get_or_create_notations(note)
        f_attrs = {'type': type}
        f = etree.SubElement(notations, 'fermata', **f_attrs)
    ...
    _SIMPLE_ORNAMENTS = {
        ...
        # nachshlang ≈ schleifer (Baroque slide ornament)
        'nachshlang': 'schleifer',
        'schleifer':  'schleifer',
    }

    def add_ornament(self, note, ornament_type: str, **attrs):
        """
        Attach an ornament marker to an existing note element.

        Recognised simple ornaments (emitted under <notations><ornaments>):
          'trill', 'turn', 'shake', 'inverted-turn',
          'mordent', 'inverted-mordent', 'nachshlang' / 'schleifer'.

        Special cases (NOT routed through this method — use the dedicated
        helper instead):
          'acciaccatura'      → use add_grace_note(slash=True) before the
                                principal note.
          'long_appoggiatura' → use add_grace_note(slash=False) before the
                                principal note.
          'glissando'         → use add_glissando(start_note, stop_note).

        Returns the inserted child element.
        """
        ornament_key = ornament_type.lower()
        if ornament_key not in self._SIMPLE_ORNAMENTS:
            raise ValueError(
                f"Ornament '{ornament_type}' is not a simple notations entry. "
                "Use add_grace_note() for acciaccatura/long_appoggiatura, or "
                "add_glissando() for glissando."
            )
        tag = self._SIMPLE_ORNAMENTS[ornament_key]
        ornaments = self._get_or_create_ornaments(note)
        str_attrs = {k: str(v) for k, v in attrs.items()}
        return etree.SubElement(ornaments, tag, **str_attrs)

    def add_glissando(self,
                      start_note,
                      stop_note,
                      number: int = 1,
                      line_type: str = 'wavy'):
        """
        Mark a glissando spanning two notes.

        line_type — 'wavy' for the wavy glissando line (most common) or
                    'solid' / 'dashed' / 'dotted'.
        """
        n_start = self._get_or_create_notations(start_note)
        etree.SubElement(n_start, 'glissando',
                         type='start',
                         number=str(number),
                         **{'line-type': line_type})
        n_stop = self._get_or_create_notations(stop_note)
        etree.SubElement(n_stop, 'glissando',
                         type='stop',
                         number=str(number),
                         **{'line-type': line_type})
        return (n_start, n_stop)

    def add_repeat(self, measure, direction: str = 'backward',
                   location: Optional[str] = None,
                   style: Optional[str] = None,
                   winged: Optional[str] = None):
        """
        Convenience: add a <barline><repeat direction="..."/></barline>.

        direction — 'forward' (start-repeat) or 'backward' (end-repeat).
        location  — defaults to 'left' for forward and 'right' for backward
                    if not given.
        style     — bar-style text (e.g. 'heavy-light' for forward repeats).
                    Defaults to 'heavy-light' for forward and 'light-heavy'
                    for backward when not specified.
        """
        if location is None:
            location = 'left' if direction == 'forward' else 'right'
        if style is None:
            style = 'heavy-light' if direction == 'forward' else 'light-heavy'
        return self.add_barline(measure,
                                 location=location,
                                 style=style,
                                 repeat=direction,
                                 repeat_winged=winged)
