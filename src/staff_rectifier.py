...
def process_image(image_path: str,
                  output_dir: str,
                  target_width: Optional[int] = None,
                  target_line_spacing: Optional[int] = None,
                  vertical_pad: Optional[int] = None,
    ...
    """
    See main docstring below.  ``return_metadata=True`` causes this
    function to return ``(rectified_image, staves_meta)`` instead of
    just the image, where ``staves_meta`` is a list of dicts giving
    each staff's location IN THE RECTIFIED CANVAS:

        {'top_y': int, 'bot_y': int,
         'left_x': int, 'right_x': int,
         'line_spacing': float,
         'line_positions': [y1, y2, y3, y4, y5]}

    This lets downstream stages skip a second round of staff detection.
    """
    """
    ...
    # ── Step 1b: extend single-staff rows to full image width ────────
    # The U-Net's left_x/right_x sometimes clip the start of a clef or
    # the trailing barline.  Per the user's design: when a row of the
    # page contains exactly ONE detected staff, we treat the whole row
    # as one stave and pull its extent to the full image width so
    # nothing is cropped during the homography.  Rows that contain
    # multiple detected staves (real side-by-side systems, or a single
    # staff the detector split in two) keep their detected extents so
    # we don't accidentally merge two sub-staves.
    #
    # (Future refinement: when a row has multiple staves and we have
    # clef-x hints, we can place each sub-stave's left edge a few
    # pixels left of its clef; pass them via the ``clef_x_hints``
    # kwarg.)
    if len(staves) >= 1:
        # Staff dicts here use polylines (top_curve / bottom_curve) plus
        # left_x / right_x / line_spacing — no direct top_y / bot_y, so
        # we derive vertical position from the median y of each curve.
        def _curve_med_y(curve):
            return float(np.median([p[1] for p in curve])) if curve else 0.0

        top_ys = [_curve_med_y(s.get('top_curve') or []) for s in staves]
        bot_ys = [_curve_med_y(s.get('bottom_curve') or []) for s in staves]
        heights = [max(1.0, b - t) for t, b in zip(top_ys, bot_ys)]
        median_h = float(np.median(heights))
        row_tol  = max(8.0, median_h * 0.5)

        sorted_idxs = sorted(range(len(staves)), key=lambda i: top_ys[i])
        ...
        for idx in sorted_idxs:
            ...
            if anchor is None or abs(top - anchor) <= row_tol:
                ...
                anchor = float(np.mean([top_ys[i] for i in row_groups[-1]]))
            else:
        ...
        if verbose and n_extended:
            print(f'      extended {n_extended}/{len(staves)} single-staff '
                  f'rows to full image width')

    # ── Auto-scale sizing from the detected source geometry ──────────
    # If the caller didn't override these, derive sensible defaults
    # from the input so the rectified canvas keeps the page's natural
    # proportions.  Without this the output uses fixed pixel values
    # (target_line_spacing=18, vertical_pad=100, gap=80) and a low-res
    # input ends up "squished" — the per-staff bands take far more
    # vertical space than the original staves did.
    src_spacings = [float(s['line_spacing']) for s in staves
                    if 'line_spacing' in s and s['line_spacing'] > 0]
    if src_spacings:
        median_ls = float(np.median(src_spacings))
    else:
        median_ls = float(TARGET_LINE_SPACING)
    if target_line_spacing is None:
        target_line_spacing = max(6, int(round(median_ls)))
    if vertical_pad is None:
        # ~4× the line spacing gives room for ledger lines, stems,
        # AND ornaments that sit further from the staff — fermatas,
        # dynamics, accents.  At ls=9 this lands at 36 px above and
        # below the staff (vs. 18 with the old 2× multiplier), which
        # comfortably contains fermatas that previously got clipped.
        vertical_pad = max(24, int(round(4.0 * target_line_spacing)))
    if gap is None:
        gap = max(6, int(round(1.5 * target_line_spacing)))
    if verbose:
        print(f'      sizing: target_line_spacing={target_line_spacing} '
              f'(src median={median_ls:.1f}), vertical_pad={vertical_pad}, '
              f'gap={gap}')
    ...
    # ── Compute per-staff canvas-coordinate metadata for downstream ───
    # Mirrors the geometry inside rectify_cut() + stitch_staves():
    #   each rectified staff image has the 5 stafflines at
    #     y_local = vertical_pad + k * target_line_spacing  (k = 0..4)
    #   and the staff content spans
    #     x_local in [horizontal_pad, w_rect - horizontal_pad]
    horizontal_pad = 30  # matches rectify_cut() default
    rect_heights = [r.shape[0] for r in rectified]
    if preserve_layout:
        canvas_x_offsets = [max(0, x_off - 30) for x_off in cut_x_offsets]
    else:
        canvas_x_offsets = [0] * len(rectified)

    staves_meta: List[dict] = []
    y_running = 0
    for i, (rect_img, x_off) in enumerate(zip(rectified, canvas_x_offsets)):
        h_rect, w_rect = rect_img.shape[:2]
        line_positions = [y_running + vertical_pad + k * target_line_spacing
                          for k in range(5)]
        left_x  = x_off + horizontal_pad
        right_x = x_off + max(horizontal_pad + 1, w_rect - horizontal_pad)
        staves_meta.append({
            'top_y':          line_positions[0],
            'bot_y':          line_positions[-1],
            'left_x':         int(left_x),
            'right_x':        int(right_x),
            'line_spacing':   float(target_line_spacing),
            'line_positions': line_positions,
        })
        y_running += h_rect + gap

    if return_metadata:
        return final, staves_meta
    return final
