...
def analyze_score(image_path: str,
    ...
    """
    Main entry point.  Detects staves on a page from scratch and
    returns a fully-assembled ScoreData.
    """
    ...
    return assemble_score_data(
        image_path               = image_path,
        img_color                = img_color,
        staves_meta              = staves_meta,
        num_parts                = num_parts,
...
def assemble_score_data(image_path: str,
                        ...
                        horizontal_padding_ratio: float = 0.05) -> ScoreData:
    """
    Build a ``ScoreData`` from pre-computed staff metadata.

    This is the "lower half" of ``analyze_score`` factored into its own
    entry point so callers that already know where the staves are
    (e.g. the rectifier in Stage 1) can skip re-detection.

    ``staves_meta`` is a list of dicts shaped exactly like the one
    ``analyze_score`` builds internally::

        {'top_y', 'bot_y', 'left_x', 'right_x',
         'line_spacing', 'line_positions'}

    The function handles row clustering, part assignment (side-by-side
    vs. vertically-stacked layout), per-staff cropping, and finally
    wraps everything in ``StaffData`` / ``PartData`` / ``ScoreData``.
    """
    ...
    # Choose the assignment strategy based on the row layout.
    #
    # CASE A — most rows have ≥ 2 staves → side-by-side systems within
    #          each row.  Use position-within-row modulo num_parts.
    # CASE B — most rows have exactly 1 staff → vertically-stacked parts
    #          (e.g. 4-clarinet score with each clarinet on its own line).
    #          The whole staff list, sorted top-to-bottom, becomes a
    #          sequence of "systems" of size num_parts: top-most staff
    #          goes to P1, next to P2, …, wrapping around with modulo.
    multi_staff_rows = sum(1 for r in rows if len(r) >= 2)
    use_stacked_layout = (multi_staff_rows == 0 and num_parts > 1)
    if use_stacked_layout:
        print(f'[score_analyzer] vertically-stacked layout detected '
              f'({len(rows)} single-staff rows, num_parts={num_parts}); '
              f'distributing top-to-bottom across parts.')
        # Flat list of staff indices in top-to-bottom order (each row
        # already holds one staff and rows themselves are sorted by y).
        flat_indices: List[int] = [r[0] for r in rows]
        # Map each staff to a (row_idx, part_idx) by global index modulo.
        global_to_assignment: dict = {}
        for global_i, st_idx in enumerate(flat_indices):
            part_idx = global_i % num_parts
            system_idx = global_i // num_parts
            global_to_assignment[st_idx] = (system_idx, part_idx)
    else:
        global_to_assignment = None
    ...
    for row_idx, row in enumerate(rows):
        ...
        for pos, idx in enumerate(row):
            ...
            if global_to_assignment is not None:
                _, part_idx = global_to_assignment[idx]
            else:
...
if __name__ == '__main__':
    ...
    print(f'Total staves: {len(score.all_staves)}')

    for part in score.parts:
        print(f'\n{part.part_id} ({len(part.staves)} staves):')
        for staff in part.staves:
            print(f'  row={staff.row_idx}  '
                  f'y={staff.top_y}-{staff.bot_y}  '
                  f'x={staff.left_x}-{staff.right_x}  '
                  f'crop_x={staff.crop_x1}-{staff.crop_x2}  '
    ...
    for staff in score.all_staves:
        path = os.path.join(out_dir,
            f'r{staff.row_idx}_{staff.part_id}_staff{staff.staff_in_part+1:02d}.png')
