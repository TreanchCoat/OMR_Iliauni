...
from score_analyzer import (
    analyze_score, assemble_score_data, ScoreData, StaffData,
)
...
def preprocess_image(image_path: str,
                     ...
                     precomputed_staves: Optional[List[dict]] = None
                     ) -> ProcessedScore:
    """
    ...
    Steps:
        1. Detect staff lines and group into parts (score_analyzer)
            — SKIPPED when ``precomputed_staves`` is provided.  In that
              case the supplied metadata is used directly so we don't
              waste cycles re-detecting on an already-rectified page.
    ...
    Args:
        ...
        precomputed_staves: optional list of staff-position dicts from
                            an upstream detector (e.g. ``staff_rectifier
                            .process_image(..., return_metadata=True)``).
    ...
    if precomputed_staves is not None:
        # Skip detection — use the metadata Stage 1 already produced.
        # Read the rectified page so we can crop each staff out of it.
        img_color = cv2.imread(str(image_path))
        if img_color is None:
            raise FileNotFoundError(f'Cannot read image: {image_path}')
        score = assemble_score_data(
            image_path  = image_path,
            img_color   = img_color,
            staves_meta = precomputed_staves,
            padding_ratio = padding_ratio,
        )
        print(f'Using {len(precomputed_staves)} precomputed staves from Stage 1.')
    else:
        # Step 1: Detect and group staves (legacy path).
