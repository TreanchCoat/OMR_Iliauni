...
class XMLBuilder:
    ...
    def _get_or_create_identification(self):
        ...
        Identification must be the FIRST child of <score-partwise>."""
        ident = self._root.find('identification')
        if ident is None:
            ident = etree.Element('identification')
            self._root.insert(0, ident)
        return ident
...
if __name__ == '__main__':
    ...
    m3 = builder.add_measure('P1', 3, width=317)
    builder.add_note(m3, step='F', octave=4, duration=8, duration_type='half',
    ...
    m_last = builder.add_measure('P1', 15, width=275)
