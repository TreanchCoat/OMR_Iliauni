...
def visualize_detections(processed_score, page_detections,
    ...
    use_cleaned       If True, draw on the staff-line-removed binary image
                      instead of the original color crop.  Color crop is
    ...
    for part_staves, staff_dets_list in zip(processed_score.parts,
        ...
        for pstaff, sdet in zip(part_staves, staff_dets_list):
            base = pstaff.cleaned if use_cleaned else pstaff.crop
            if base.ndim == 2:
            ...
            # Header
            header = (f'{sdet.part_id}  staff {sdet.staff_in_part + 1}  '
                      f'({sdet.total_detections} detections)')
