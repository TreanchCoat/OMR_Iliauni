...
# Bootstrap: register <project>/src on sys.path so env_loader is importable
# whether dummy_api.py is run from <project>, <project>/api, or elsewhere.
...
_SAMPLE = env_loader.SAMPLE_DATA_DIR

# Fixture file paths — sourced from env_loader (which reads .env).  Drop
# matching files into data/sample/ or override DUMMY_* in .env.
RECTIFIED_PATH  = Path(env_loader.DUMMY_RECTIFIED_PATH)
XML_PATH        = Path(env_loader.DUMMY_XML_PATH)
DETECTIONS_PATH = Path(env_loader.DUMMY_DETECTIONS_PATH)

# Fall back to legacy filenames the dummy server used to ship with.
if not RECTIFIED_PATH.exists():
    for candidate in [_SAMPLE / 'rectified.png',
                       _SAMPLE / '01_Okribuli_makruli.png']:
        if candidate.exists():
            RECTIFIED_PATH = candidate
            break

if not XML_PATH.exists():
    for candidate in [_SAMPLE / '01_Okribuli_makruli.xml',
                       _SAMPLE / 'score.xml']:
        if candidate.exists():
            XML_PATH = candidate
            break
...
if not DETECTIONS_PATH.exists():
    for candidate in [_SAMPLE / 'P1_staff01.json',
                       _SAMPLE / 'detections.json']:
        if candidate.exists():
            DETECTIONS_PATH = candidate
            break

MAX_IMAGE_MB = env_loader.MAX_IMAGE_MB
HOST         = env_loader.HOST
PORT         = env_loader.PORT
